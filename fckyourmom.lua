...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>Succubus Hub chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "Succubus Hub", HidePremium = false, IntroText = "Succubus Hub⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("Succubus Hub SCRIPT")
W:AddLabel("DiscordSuccubus Hub")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "Succubus Hub(Noitification)",
